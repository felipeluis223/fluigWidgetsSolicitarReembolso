...
- Sempre utilizar o seguinte modelo:
	 ...
	 * @example Exemplo funcional de como utilizar a função. Ex.: FLUIGS.writeForState();
...
function FLUIGS(){
	 ...
	 * @memberof FLUIGS
	 * @example FLUIGS.writeForState();
	 * @example FLUIGS, campo obrigatório, campo válido, readOnly, state
	 * @example
	 * @author
     */
		...
		$("input,select,textarea").each(function(){
			...
			if ( (typeof required != 'undefined') && (typeof states != 'undefined') ){
				...
				if ( eval(required) && (states.indexOf(getWKNumState()) != -1  || states.indexOf("*") != -1) ) {
					...
					FLUIGS.execChangeFields($(this), false);
				}
			}
		});
	}
	 ...
	 * @memberof FLUIGS
	 * @example FLUIGS.setReadOnlyByStates();
	 * @example FLUIGS, fluig-state-readonly, readOnly, state
	 * @example
	 * @author
     */
	 ...
	 * @memberof FLUIGS
	 * @example FLUIGS.setHideElementsByStates();
	 * @example FLUIGS, state, fluig-state-hide,
	 * @example
	 * @author
     */
	//FIXME: Não testado
	this.setHideElementsByStates = function(){
		...
		$("[data-fluig-state-hide]").each(function () {
			...
			var keep = FLUIGS.validConditional(conditional);
	 ...
	 * @example FLUIGS.setWriteFieldByClass('.form-control');
	 * @example FLUIGS, readOnly, classe
	 * @example
	 * @author
     */
	 ...
	 * @memberof FLUIGS
	 * @example FLUIGS.removeAlertValidation();
	 * @example FLUIGS, validação
	 * @example
	 * @author
     */
	 ...
	 * @memberof FLUIGS
	 * @example FLUIGS.setLabelRequiredByStates();
	 * @example FLUIGS, state, control-label, fluig-required
	 * @example
	 * @author
     */
	 ...
	 * @memberof FLUIGS
	 * @example FLUIGS.setWriteFieldByState();
	 * @example FLUIGS, state, readOnly, fluig-write
	 * @example
	 * @author
     */
	 ...
	 * @example FLUIGS.setReadOnly(".form-control"); FLUIGS.setReadOnly("#userName");
	 * @example FLUIGS, readOnly, classe
	 * @example
	 * @author
     */
	this.setReadOnly = function(selector) {
		$(selector).each(function () {
			FLUIGS.execChangeFields($(this), true);
		});
	}
	 ...
	 * @example FLUIGS.execChangeFields($(this), false);
	 * @example FLUIGS, readOnly
	 * @example
	 * @author
     */
	 ...
	 * @returns {boolean}  True | False
	 * @example variavelTemTipo = FLUIGS.validConditional(variavel);
	 * @example FLUIGS, válido, validação, verificação
	 * @example
	 * @author
     */
     ...
     * @param {*} conditional Variável a ser verificada
	 * @returns {boolean}  True | False
	 * @example variavelTemTipo = FLUIGS.isValid(variavel);
	 * @example FLUIGS, válido, validação, verificação
	 * @example
	 * @author
     */
	 ...
	 * @memberof FLUIGS
	 * @example FLUIGS.updCalendar();
	 * @example FLUIGS, readOnly, calendário
	 * @example
	 * @author
     */
	...
	* @example FLUIGS.setSwitcher("#id"); FLUIGS.setSwitcher(".bonecas");
	* @example FLUIGS, checkbox, radio button, switcher, set, campo
	* @example
	...
	* @example FLUIGS.setSwitcherByName("genero");
	* @example FLUIGS, checkbox, radio button, switcher, set, name, campo
	* @example
	* @author Andressa Oliveira
	*/
	...
	this.TABLE = {
		 ...
		 * @example numLinhas = FLUIGS.TABLE.getTableSize("tabelaClientes", 2);
		 ...
		 * @example FLUIGS.TABLE.clearTable("minhaTabela", "Finalizado com sucesso!");
		...
		clearTable: function(tableName,msg){
			...
			if (contador > 0) {
				 if (!FLUIGS.USEFUL.isNull(msg)) {
					 FLUIGS.USEFUL.showInfo(msg);
				 }
			}
		},
		 ...
		 * @example FLUIGS.TABLE.removeLineTable(oElement);
		 * @example TABLE, deletar, tabela, linhas
		 * @example
		 * @author
		 */
		 ...
		 * @example FLUIGS.TABLE.addTable(table);
		 ...
		 * @example FLUIGS.TABLE.addTableChild(table);
		 * @example TABLE, adicionar, tabela
		 * @example
		 * @author
		 */
	...
	this.USEFUL = {
		...
		validFloatValue: function(value){
			...
			// is a number
			if (!isNaN(FLUIGS.USEFUL.replaceAll(value,',',''))) {
				valid = FLUIGS.USEFUL.getFloatValue(value) == 0;
			}
		 ...
		 * @example nomeSemAcentos = FLUIGS.USEFUL.retiraAcentos(nome);
		 ...
		 * @example estaVazia = FLUIGS.USEFUL.empty(data);
		...
		empty: function(element){
			if (!FLUIGS.USEFUL.isNull(element)) {
		 ...
		 * @returns {boolean} Retorna true ou false
		 * @example estaVazia = FLUIGS.USEFUL.isEmpty(data);
		 * @example USEFUL, validação, válido, vazio, verificação
		 * @example
		 * @author
		 */
		 ...
		 * @returns {boolean}  Retorna True ou False
		 * @example resposta = FLUIGS.USEFUL.isNull(variavel);
		 ...
		 * @example FLUIGS.USEFUL.setValueDisableField("telefone", "12 1234-5678");
		 * @example USEFUL, alteração, readOnly
		 * @example
		 * @author
		 */
		setValueDisableField: function(fieldName, value) {
			FLUIGS.setWriteFieldByClass(fieldName);
			$(fieldName).val(value);
			FLUIGS.setReadOnly(fieldName);
		},
		 ...
		 * @example dataAtual = FLUIGS.USEFUL.getDate();
		 ...
		 * @example horaAtual = FLUIGS.USEFUL.getTime();
		 ...
		 * @example retorno = FLUIGS.USEFUL.mValor(valor);
		 ...
		 * @example retorno = FLUIGS.USEFUL.mInt(valor);
		 * @example USEFUL, formatação, regex, verificação, números,
		...
		*/
		 ...
		 * @example retorno = FLUIGS.USEFUL.mNumerico(valor);
		 ...
		 * @param {string} valor String a ser padronizada
		 * @returns {string} String com o novo padrão
		 * @example retorno = FLUIGS.USEFUL.mChar(valor);
		 ...
		 * @param {string} valor String a ser padronizada
		 * @returns {string} String com o novo padrão
		 * @example retorno = FLUIGS.USEFUL.mAlfanum(valor);
		 ...
		 * @example dataInicial = FLUIGS.USEFUL.stringToDate('01/01/2000');
		 ...
		 * @example FLUIGS.USEFUL.dateToString(data);
		 * @example USEFUL, conversão, string, date
		 * @example
		 ...
		 * @example precoFinal = FLUIGS.USEFUL.getStringValue(float, 2);
		 ...
		 * @example float = FLUIGS.USEFUL.getFloatValue(valor);
		 * @example USEFUL, conversão, string, float
		 * @example
		 * @author
		 */
		getFloatValue: function(string) {
			...
			string = FLUIGS.USEFUL.replaceAll(string, ".", "");
			string = FLUIGS.USEFUL.replaceAll(string, ",", ".");
		 ...
		 * @example novaString = FLUIGS.USEFUL.replaceAll(stringOriginal, fromValue, toValue);
		 ...
		 * @example idLinha = FLUIGS.USEFUL.getIdChild(nome);
		 * @example USEFUL, get
		 * @example
		 * @author
		 */
		 ...
		 * @example FLUIGS.USEFUL.showError(mensagem);
		 ...
		 * @memberof USEFUL
		 * @param {string} msg Mensagem a ser exibida
		 * @example FLUIGS.USEFUL.showSuccess(mensagem);
		 ...
		 * @memberof USEFUL
		 * @param {string} msg Mensagem a ser exibida
		 * @example FLUIGS.USEFUL.showWarning(mensagem);
		 ...
		 * @memberof USEFUL
		 * @param {string} msg Mensagem a ser exibida
		 * @example FLUIGS.USEFUL.showInfo(mensagem);
		 ...
		 * @example FLUIGS.USEFUL.showMessage(mensagem, titulo, textoBotao);
		 ...
		 * @example FLUIGS.USEFUL.clearFields("#cep",".endereco"); FLUIGS.USEFUL.clearFields("#cidade","#UF");
		 ...
		 * @example FLUIGS.USEFUL.setDateRange("#meu-calendario","05/03/2020", 2, 10);
		...
		setDateRange: function(calendarVar, initialDate, daysBefore, daysAfter) {
			...
			//validações
			if (FLUIGS.USEFUL.isNull(daysBefore)) daysBefore = 0;
			if (FLUIGS.USEFUL.isNull(daysAfter)) daysAfter = 0;
			if (FLUIGS.USEFUL.isNull(initialDate)) {
				initialDate = new Date();
				dataInicial = initialDate;
				dataFinal = initialDate;

			}else{
				dataInicial = FLUIGS.USEFUL.stringToDate(initialDate);
				dataFinal = FLUIGS.USEFUL.stringToDate(initialDate);
	...
	this.VALID = {
		 ...
		 * @example FLUIGS.VALID.validateForm();
			...
			$("input,select,textarea").each(function(){
				...
				if ( (typeof required != 'undefined') && (typeof states != 'undefined') && (validConditional) ){
					...
					if ( eval(required) && (states.indexOf(getWKNumState()) != -1  || states.indexOf("*") != -1) ) {
						...
						if ( value.trim() == "" || value =='undefined' || value == null || FLUIGS.USEFUL.validFloatValue(value) ){
		 ...
		 * @returns {boolean}  Retorna True ou False
		 * @example temLinhas = FLUIGS.VALID.validateGrid('tabela');
		 ...
		 * @example eDuplicado = FLUIGS.VALID.validDuplicateValueGrid("tabelaClientes", "nomeCliente", "Maria"));
		...
		validDuplicateValueGrid: function(idTable, idField, value){
			...
			var itens = FLUIGS.TABLE.getTableSize(idTable);
			var count = 0;
			var retorno = false;
			var valor = "";
			if (!FLUIGS.USEFUL.isNull(value)) {
				for (var i = 1; i <= itens; i++) {
					valor = (FLUIGS.USEFUL.isNull(document.getElementById(idField+"___"+i))) ? "" : document.getElementById(idField+"___"+i).value;
		 ...
		 * @param {string} value Valor a ser comparado
		 * @returns {boolean} True ou false
		 * @example eDuplicado = FLUIGS.VALID.isDuplicatedOnGrid("tabelaClientes", "nomeCliente", "Maria"));
		 * @example VALID, validação, verificação, tabela, grid
		 * @example
		 * @author
		 *
		...
		isDuplicatedOnGrid: function(idTable, idField, value){
			...
			if (!FLUIGS.USEFUL.isNull(value)) {
				$("#"+idTable+" tbody tr").each(function(){
					var id = FLUIGS.USEFUL.getIdChild($(this).find("input:first").attr("name"));
					if(id > 0){
						valor = (FLUIGS.USEFUL.isNull(document.getElementById(idField+"___"+id))) ? "" : document.getElementById(idField+"___"+id).value;
						if (valor == value) {
							count++;
						}
						if (count > 1) {
	...
	this.ZOOM = {
		 ...
		 * @example FLUIGS.ZOOM.setZoomData(instancia, valor);
		 ...
		 * @example FLUIGS.ZOOM.setZoomReadonly(instancia, true);
		 ...
		 * @example FLUIGS.ZOOM.removeItemZoom(instancia);
 ...
 * @example FLUIGS, validação, verificação
 * @example
 * @author
 */
...
var beforeSendValidate = function(numState, nextState) {
	...
	var isOK = true;
	var validFields = FLUIGS.VALID.validateForm()
	isOK = validFields[0];
	if ( !isOK ){
		throw validFields[1];
	} else {
...
$(document).ready( function() {
	...
	$(document).on('change', ".retiraAcentos", function () {
		$(this).val(FLUIGS.USEFUL.retiraAcentos($(this).val()));
	});
	...
	$(document).on('keyup', '.mChar',function(){
		var valor = FLUIGS.USEFUL.mChar($(this).val());
		$(this).val(valor);
	});
	...
	$(document).on('keyup', '.mAlfanum',function(){
		var valor = FLUIGS.USEFUL.mAlfanum($(this).val());
		$(this).val(valor);
	});
	...
	$(document).on('keyup', '.mInt',function(){
		var valor = FLUIGS.USEFUL.mInt($(this).val());
		$(this).val(valor);
	});
	...
	$(document).on('blur', '.mInt',function(){
		var valor2 = 0;
		var valor  = $(this).val();
		valor  = FLUIGS.USEFUL.replaceAll(valor,'.','');
		valor2 = parseInt(valor);
		valor  = FLUIGS.USEFUL.mInt(valor2.toString());
		$(this).val(valor);
	});

	$(document).on('keyup', '.mValor',function(){
		var valor = FLUIGS.USEFUL.mValor($(this).val());
		...
		if(tam != "undefined"){
			if ($(this).val().length > tam){
				...
				valor = FLUIGS.USEFUL.mValor(valor.substr(0,$(this).attr("maxlength")-pontos));
				$(this).val(valor)
			}
		}
	});
	...
	$(document).on('keyup', '.mNumerico',function(){
		var valor = FLUIGS.USEFUL.mNumerico($(this).val());
		$(this).val(valor);
	});
...
var tdizoom = (function () {
	...
	return {
		...
		open: function (dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby) {
			...
			var current = 0;
			var ajuda = {
					'pt_BR': 'Para a busca mais detalhada digitar palavra ou parte dela no campo de busca.',
					'es': 'Para una búsqueda más detallada, escriba la palabra o parte de ella en el campo de búsqueda.',
					'en_US': 'For a more detailed search, type a word or part of the search field.'
			};

			var placeholderPesquisa={
					'pt_BR': 'Digite o texto e utilize o <Enter> para buscar',
					'es': 'Escriba el texto y use <Enter> para buscar',
					'en_US': 'Type the text and use <Enter> to search'
			};
			var selecionar = {
					'pt_BR': 'Selecionar',
					'en_US': 'Select',
			...
			html += "<input type='text' class='form-control' id='search' value='" + $("#" + type).val() + "' placeholder='pesquise'>";
			html += "</div>";
				...
				if (err) {
					...
					var readydataset = function (dataset) {
						...
						$(".table-zoom > tbody > tr").dblclick(function () {
							...
							row["inputName"] = type;
							zoommodal.remove();
							EventHandler.setSelectedZoomItem(row);
							;
							if (isHeaderHide) {
								$("#workflowview-header", window.parent.document).show();
							}
					...
					$('.zoom-close').click(function () {
						...
						//			 			$("#"+type).val($("#search").val());
						//			 			$("#"+type).trigger("change");
						EventHandler.removedZoomItem(type);
						zoommodal.remove();
						$("#" + type).focus();
					});
		...
		/**
		 * Função trigger
		 * @description Realiza o gatilho do campo zoom personalizado baseado nos parametros informados. (chama as funções padrões setSelectedZoomItem e EventHandler.removedZoomItem)
		 * @memberof TDIZOOM
		 * @param {*} dataset Dataset a ser consultado
		 * @param {*} searchby Pesquisar por
...
var tdizoomCheck = (function(){
	...
	return {
		open: function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby) {
			...
			html += "<div class='input-group'>" ;
			html += "<span class='input-group-addon'><span class='fluigicon fluigicon-search'></span></span>" ;
			html += "<input type='text' class='form-control' id='search' value='" + ($("#"+type).length > 0 ? $("#"+type).val() : "") + "' placeholder='" + translate(placeholderPesquisa) + "'>";
			html += "</div>" ;
			if (typeof miniZoom != "undefined") {
				html += "<div class='' id='loading-zoom' style='overflow-y: auto; height: 120px;'>";
			} else {
